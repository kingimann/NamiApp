...
3. Add a webhook endpoint pointing at:
   `https://okayspace-api.onrender.com/api/payments/webhook`
   subscribe to **`checkout.session.completed`**, and copy its **signing secret**
   (`whsec_…`).

## 2. Environment variables (Render → okayspace-api)
| Variable | Value |
| --- | --- |
...
- To also drive it from an external scheduler, set `CRON_SECRET` and add a **Render
  Cron Job** that calls:
  ```
  curl -X POST https://okayspace-api.onrender.com/api/payouts/run \
