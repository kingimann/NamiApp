...
3. Pick your repo. Render reads `render.yaml` and shows three resources:
   ...
   - `okayspace-api` — the Docker web service (the API)
...
4. It will prompt for a few values:
   ...
   - `EXPO_PUBLIC_BACKEND_URL` (web) → the API's URL. If you don't know it yet,
     it's `https://okayspace-api.onrender.com` (Render names it after the
...
6. Render gives you a backend URL like `https://okayspace-api.onrender.com` and a
...
**Test it:** open `https://okayspace-api.onrender.com/health` — you should see
...
```
EXPO_PUBLIC_BACKEND_URL=https://okayspace-api.onrender.com
...
```bash
curl -X POST https://okayspace-api.onrender.com/api/auth/register \
