...
Base URL: `https://okayspace-api.onrender.com/api`
Interactive docs (Swagger): `https://okayspace-api.onrender.com/docs`
OpenAPI schema: `https://okayspace-api.onrender.com/openapi.json`
...
`wss://okayspace-api.onrender.com`; everything else is plain JSON over HTTPS and
...
```bash
curl https://okayspace-api.onrender.com/api/posts/feed \
  -H "Authorization: Bearer $NAMI_KEY"
```

```js
const res = await fetch("https://okayspace-api.onrender.com/api/posts/feed", {
...
import requests
r = requests.get(
    "https://okayspace-api.onrender.com/api/posts/feed",
...
3. **Exchange the code** (server-side):
   ```bash
   curl -X POST https://okayspace-api.onrender.com/api/oauth/token \
...
4. **Get the profile**:
   ```bash
   curl https://okayspace-api.onrender.com/api/oauth/userinfo \
...
`$API` = `https://okayspace-api.onrender.com/api`. Get `$TOKEN` from login, or use a
...
const api = 'https://okayspace-api.onrender.com/api';
...
openapi-generator generate \
  -i https://okayspace-api.onrender.com/openapi.json \
...
final ch = WebSocketChannel.connect(
  Uri.parse('wss://okayspace-api.onrender.com/ws/conversations/<id>?token=<token>'),
);
