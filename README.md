...
| **Embed content** | `/pub/post/{id}`, `/pub/profile/{username}`(+`/posts`), `/pub/post-card`, `/pub/profile-card`, `/pub/content-embed.js`, `/pub/oembed` | Public JSON, themeable iframe cards, a `<script>` loader, cursor-paginated profile feed, and an **oEmbed** provider. |
...
- **Embeddable content + oEmbed** — public JSON for posts/profiles, **themeable iframe cards**, a drop-in `content-embed.js` loader, a **cursor-paginated profile feed** for building a Nami feed widget, and an **oEmbed** provider so pasted Nami links auto-expand in WordPress/Discourse/Notion. Only public content is served (no subscriber-only posts, no banned users).
